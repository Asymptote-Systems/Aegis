...
def start():
    ...
    if not IS_JUDGE0_SUPPORTED:
        ...
        for _ in track(range(10), description="⏳ Warming up DB & Redis..."):
            time.sleep(1)
        run("docker compose up -d drawio backend adminer logforge-backend logforge-frontend logforge-notifier logforge-autoupdate frontend")
    else:
...
def reset_db():
    ...
    if not IS_JUDGE0_SUPPORTED:
        ...
        for _ in track(range(10), description="⏳ Re-initializing DB & Redis..."):
            time.sleep(1)
        run("docker compose up -d --build drawio frontend backend adminer logforge-backend logforge-frontend logforge-notifier logforge-autoupdate")
    else:
...
def restore_db():
    ...
    run("docker exec -it app-db psql -U app_user -d postgres -c \"SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname = 'app_db' AND pid <> pg_backend_pid();\"")
