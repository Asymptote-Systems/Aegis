...
const formatDateAsIs = (dateString) => {
...
// StudentDashboard component with proper initialization order
export default function StudentDashboard() {
    // First, initialize all refs
    const timerRef = useRef(null);

    // Then initialize all state variables
    const [registeredExams, setRegisteredExams] = useState([]);
    const [allExamRegistrations, setAllExamRegistrations] = useState([]);
    ...
    // Get context after state initialization
    ...
    useEffect(() => {
        ...
        if (timerActive && instructionTimer > 0) {
            interval = setInterval(() => {
                setInstructionTimer(timer => timer - 1);
            }, 1000);
        } else if (instructionTimer === 0) {
            setTimerActive(false);
        }
        return () => clearInterval(interval);
    }, [timerActive, instructionTimer]);
    ...
    const handleStartActivity = (assignment) => {
        // Add state data to navigation
        navigate(`/student/activity/${assignment.activity.id}`, {
            state: {
                activityData: assignment.activity,
    ...
    const fetchRegisteredExams = async () => {
        try {
            ...
            const nonSubmittedRegistrations = examRegistrations.filter(registration => {
                ...
                const allowedStatuses = ['scheduled', 'active', 'pending']; // only show these

                const isAllowed = allowedStatuses.includes(status);

                if (!isAllowed) {
                    console.log('Filtering out exam:', registration.exam_id, 'status:', registration.status);
                }
    ...
    // FIXED: Use the correct endpoint '/me/assignments/' instead of '/me/activity-assignments/'
    const fetchAssignments = async () => {
        try {
            ...
            toast.error("Failed to load assignments", {
                description: "Could not fetch your activity assignments. The endpoint may not be available."
            });
        } finally {
    ...
    function handleStartExamClick(examData) {
        ...
        setInstructionTimer(10); // Reset timer to 10 seconds
    ...
    async function handleStartExam() {
        ...
        try {
            ...
            // Determine redirect path based on exam type
            const examType = selectedExamData.exam.exam_type?.toLowerCase();
            let redirectPath;

            switch (examType) {
                case 'midterm':
                case 'final':
                    // Redirect to current coding platform
                    redirectPath = `/student/platform/${selectedExamId}`;
                    break;
                case 'practice':
                    // Redirect to practice platform (placeholder)
                    redirectPath = `/student/PracticePlatform/${selectedExamId}`;
                    break;
                case 'quiz':
                    // Redirect to MCQ platform (placeholder)
                    redirectPath = `/student/MCQPlatform/${selectedExamId}`;
                    break;
                default:
                    // Default to current coding platform
                    redirectPath = `/student/platform/${selectedExamId}`;
                    break;
            }
    ...
    // Helper to get badge variant for assignment status
    ...
    return (
        <div className="min-h-screen bg-gradient-to-b from-gray-50 via-white to-blue-50">
            ...
            <main className="max-w-8xl mx-auto px-2 sm:px-6 lg:px-8 py-8">
                ...
                {/* Tabs Container for Exams and Assignments */}
                <Tabs defaultValue="exams" className="w-full">
                    <TabsList className="grid w-full grid-cols-2">
                        <TabsTrigger value="exams">My Exams</TabsTrigger>
                        <TabsTrigger value="assignments">Activity Assignments</TabsTrigger>
                    </TabsList>
                    ...
                    <TabsContent value="assignments" className="space-y-6">
                        <Card>
                            <CardHeader>
                                ...
                                <CardDescription>Your assigned activity sheets and worksheets</CardDescription>
                            </CardHeader>
                            <CardContent>
                                {loadingAssignments ? (
                                    <div className="text-center py-10">
                                    ...
                                    <div className="text-center py-8">
                                        ...
                                        <p className="text-gray-500">You don't have any assigned activities at the moment.</p>
                                    </div>
                                ) : (
                                                ...
                                                <CardContent className="p-4">
                                                    <div className="flex items-start justify-between">
                                                        <div className="flex-1">
                                                            <h3 className="font-semibold text-lg">{assignment.activity?.title || "Untitled Activity"}</h3>
                                                            <p className="text-gray-600 mt-1">{assignment.activity?.description || "No description available"}</p>
                                                            <div className="flex items-center gap-4 mt-3 text-sm text-gray-500">
                                                                <span className="flex items-center gap-1">
                                                                    <Clock className="h-4 w-4" />
                                                                    Due: {assignment.activity?.end_time ? formatDateAsIs(assignment.activity.end_time) : "Not specified"}
                                                                </span>
                                                                <span className="flex items-center gap-1">
                                                                    <Trophy className="h-4 w-4" />
                                                                    Max Score: {assignment.activity?.max_score || "Not specified"}
                                                                </span>
                                                            </div>
                                                        </div>
                                                        <div className="flex flex-col items-end gap-2">
                                                            ...
                                                            <div className="flex gap-2">
                                                                {assignment.activity?.activity_sheet_pdf && (
                                                                    <Button size="sm" variant="outline" asChild>
                                                                        <a href={`/activities/${assignment.activity.id}/pdf/`} target="_blank" rel="noopener noreferrer">
                ...
                <AlertDialog open={showStartDialog} onOpenChange={setShowStartDialog}>
                    <AlertDialogContent className="max-w-4xl max-h-[95vh] overflow-y-auto">
                        <AlertDialogHeader>
                            <AlertDialogTitle className="text-xl font-bold">Start Exam - Important Instructions</AlertDialogTitle>
                            <AlertDialogDescription className="space-y-4 text-sm">
                                <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
                                    <p className="font-semibold text-blue-900 mb-2">You are about to start:</p>
                                    <p className="text-blue-800">
                                        <strong>{selectedExamData?.exam?.title}</strong> ({selectedExamData?.exam?.exam_type || 'exam'})
                                    </p>
                                    <p className="text-blue-700 mt-1">
                                        <strong>Duration:</strong> {selectedExamData?.exam?.duration_minutes} minutes
                                    </p>
                                </div>
                                ...
                                <div className="bg-amber-50 p-4 rounded-lg border border-amber-200">
                                    <h4 className="font-bold text-amber-900 mb-3 flex items-center">
                                        ...
                                        CODING REQUIREMENTS - READ CAREFULLY
                                    </h4>
                                    <ul className="space-y-2 text-amber-800">
                                        <li className="flex items-start">
                                            <span className="font-bold mr-2">1.</span>
                                            <span><strong>Write Complete Code:</strong> You must write complete, executable code for each problem. Incomplete code will not be evaluated.</span>
                                        </li>
                                        <li className="flex items-start">
                                            <span className="font-bold mr-2">2.</span>
                                            <span><strong>Handle Input/Output:</strong> Your code must read input using standard input methods (like input() in Python, Scanner in Java, cin in C++) and print output using standard output (print(), System.out.println(), cout, etc.).</span>
                                        </li>
                                        <li className="flex items-start">
                                            <span className="font-bold mr-2">3.</span>
                                            <span><strong>No Interactive Input:</strong> Do not include prompts like "Enter number:" when taking input. Your code should directly read the input values.</span>
                                        </li>
                                        <li className="flex items-start">
                                            <span className="font-bold mr-2">4.</span>
                                            <span><strong>Exact Output Format:</strong> Your output must match the expected format exactly. Extra spaces, wrong formatting, or additional text will result in incorrect evaluation.</span>
                                        </li>
                                        <li className="flex items-start">
                                            <span className="font-bold mr-2">5.</span>
                                            <span><strong>Solve Generally, Not Just Examples:</strong> Do NOT hardcode solutions based on the example test cases shown in the problem. Your code will be tested against different, hidden test cases. Write logic that works for all valid inputs, not just the examples.</span>
                                        </li>
                                    </ul>
                                </div>

                                <div className="bg-green-50 p-4 rounded-lg border border-green-200">
                                    <h4 className="font-bold text-green-900 mb-2">Example:</h4>
                                    <div className="bg-white p-3 rounded border text-sm font-mono">
                                        ...
                                        <div className="text-red-700 mt-2">
                                            ...
                                            <code>n = int(input("Enter number: "))<br />
                                                print("Result is:", n * 2)</code>
                                        </div>
                                    </div>
                                </div>
