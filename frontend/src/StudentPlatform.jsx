...
import { Clock, FileCheck, AlertTriangle, Save, CheckCircle, Moon, Sun, LogOut, Trophy, Calendar, RotateCcw, Play, FileText } from 'lucide-react';
...
const Student_UI = () => {
  ...
  const handleSubmitExam = useCallback(async () => {
    ...
    try {
      ...
      const updateData = {
        status: "submitted",
        approved_at: new Date().toISOString(), // current timestamp in UTC
        approved_by: examCreatedByRef.current, // teacher’s ID
        extra_data: {}
      };
      ...
      if (!response.ok) {
        ...
        toast.error("Failed to update exam registration status");
      ...
      setTimeout(() => {
        ...
        navigate("/login", {
          ...
          state: {
            message:
              "Your exam has been submitted successfully. Please login again to access your dashboard.",
            type: "success"
          }
        });
      }, 10000);
  ...
  }, [performAutoSave, storageKey, logout, navigate, calculateTimeRemaining, getCurrentUser]);
    ...
    try {
      ...
      if (response.ok) {
        ...
        const currentRegistration = registrations.find(reg => 
          reg.exam_id === examId && reg.student_id === currentUserIdRef.current
        );
        ...
        if (currentRegistration) {
          ...
          console.warn('No exam registration found for current user and exam', {
            ...
            registrations: registrations.map(r => ({ id: r.id, exam_id: r.exam_id, student_id: r.student_id }))
          });
        }
      } else {
    ...
    const fetchExamData = async () => {
      ...
      try {
        ...
        if (user) {
          ...
          setFetchError('Failed to get user information. Please login again.');
          setIsLoading(false);
          return;
        }

        // Ensure user ID is set before fetching registration
        if (!currentUserIdRef.current) {
          console.error('User ID not set in ref after getCurrentUser');
          setFetchError('User ID not available. Please refresh and try again.');
          setIsLoading(false);
          return;
        }
        ...
        const questions = questionsData
          .map((q, index) => ({
            ...
            statement_type: q.question.statement_type || 'html', // ADD THIS LINE
              ...
              <div className="flex-1 overflow-hidden">
                {showRunOutput && runResult ? (
                  ...
                  <ResizablePanelGroup direction="vertical" className="h-full">
                    ...
                    {/* Resize Handle - This creates the draggable border */}
                    <ResizableHandle withHandle />
              ...
              <div className="flex-shrink-0 flex justify-between gap-2 p-2 border-t">
                <div className="flex gap-2">
                  ...
                  <Button
                    ...
                    disabled={isRunning || !code.trim() || isCurrentQuestionSubmitted}
